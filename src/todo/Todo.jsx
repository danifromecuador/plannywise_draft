...
export const Todo = ({ store, title, todos, dones }) => {
  ...
  }, [store.todos, store.dones, todos, dones]);

  const calculateCompletedPercentage = () =>
    Math.floor(
      (store.dones.length / (store.todos.length + store.dones.length)) * 100
    );
    ...
    <div className="todo">
      <div className="header">
        <span className="todo-completed">
          <span className={isNaN(calculateCompletedPercentage()) ? "hide" : ""}>
            Completed:{" "}
          </span>
          <span
            className={
              isNaN(calculateCompletedPercentage())
                ? "hide"
        ...
        <h1>{title}</h1>
        <div className={`todo-collapse-up ${hide1}`} onClick={collapse}>
          <div className="arrow-up"></div>
        </div>
        <div className={`todo-collapse-down ${hide2}`} onClick={collapse}>
          <div className="arrow-down"></div>
        </div>
      </div>
      <ul
        className={` ${collapseClass} ${
          store.todos.length === 0 && store.dones.length === 0 ? "hide" : ""
        }`}
      >
        {store.todos.map((t) => (
          <li
